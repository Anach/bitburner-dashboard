...
The **Start Order** list itself shows each service's own RAM next to its entry, with a `(Children - X GB)` suffix for the scripts declared in `capacityPlanningScripts` (older Home-only descriptors fall back to `managedScripts`). The entry and suffix show parent and child costs side by side; this capacity field is deliberately separate from `managedScripts`/`managedNetworkScripts`, which control lifecycle and placement. The card title shows **Combined total**, the sum of every listed service plus its declared children, regardless of whether each one (or an optional sub-toggle inside it) is currently switched on or which rooted host will carry it. This is a service-stack capacity ceiling, not a live snapshot of what's running right now — the Service Startup RAM Limit above answers that question instead and deliberately excludes children.
...
BBS Connect is a self-contained workspace plugin under `dashboard/plugins/bbs-connect/`, with a standalone tail launcher at `bbs-connect-standalone.js`. The public plugin contains no games; an empty seed example is provided at `dashboard/examples/example-bbs-connect-seed.json`. A private installation may supply `dashboard/plugins/bbs-connect/bbs-connect-seed.json`, which BBS Connect copies once into its player-owned `data/bbs-connect-connect-seed.json` runtime catalog. Its **C: CATALOG** screen imports exact HTTPS QuickServe source-file links, validates each source in a temporary session, and requires confirmation after displaying the detected title. Imported games retain cached source snapshots, can be hidden, shown, or removed from the catalog, and are never silently updated. Hand-maintained local QuickServe sources belong under `data/bbs-connect-connections/*.bbs.txt`, so they survive plugin updates. The catalog schema is [documented here](docs/schemas/bbs-connect-connections.schema.json). Native games use the public no-op `bbs-connect-user-applications.js` registry; a private staging source may replace it with cabinet adapters, while an absent override imports nothing.

For example, restoring a private Zork connection means installing and maintaining the Zork package yourself, then adding its cabinet/loader adapter to that registry. It is intentionally not a public compatibility promise:

```js
import { Cabinet as ZorkCabinet } from "software/zork/cabinet";
import { loadZorkResources } from "software/zork/zork-service";

export function createBbsConnectUserApplications() {
    return [{
        id: "zork",
        label: "ZORK",
        protocol: "z-machine",
        status: "LOCAL",
        description: "A privately installed Zork cabinet.",
        component: ZorkCabinet,
        load: loadZorkResources,
    }];
}
```

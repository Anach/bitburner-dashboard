...
// Points/scales/SVG path strings are pure functions of (section, terminalMode) - `section` is
// itself already reference-stable when its telemetry is unchanged (see getPluginIntegrationGraphs/
// getPluginIntegrationSections). A React tree only lives between ns.printRaw() calls in this
// dashboard - there is no persisted fiber tree across ticks - so React.memo/useMemo cannot help
// here; the cache must be module-level, keyed by the section object itself (a WeakMap, since
// multiple graphs can be on screen and computed in the same render pass).
const cachedGraphGeometryBySection = new WeakMap();

function computeGraphGeometry(section, terminalMode) {
    // WeakMap keys must be objects - fall back to always-recompute for a missing/invalid section
    // rather than throwing.
    const cacheable = section !== null && typeof section === "object";
    if (cacheable) {
        const cached = cachedGraphGeometryBySection.get(section);
        if (cached && cached.terminalMode === terminalMode) {
    ...
    if (points.length === 0 || series.length === 0 || yValues.length === 0) {
        ...
        geometry = {
            empty: false,
            points, graphWidth, graphHeight, plot, xMin, xMax, yMin, yMax, gridTicks, renderedSeries,
            zeroY: scaleY(0),
        };
    }

    if (cacheable) cachedGraphGeometryBySection.set(section, { terminalMode, geometry });
    return geometry;
}
...
export function DataGraph({ section, index = 0, presentation = "default", offline = false, sourceLabel }) {
    ...
    const geometry = computeGraphGeometry(section, terminalMode);
    ...
    const { points, graphWidth, graphHeight, plot, xMin, xMax, yMin, yMax, gridTicks, renderedSeries, zeroY } = geometry;
    ...
    return <div data-dashboard-theme-role="graph-panel" style={frameStyle}>
        ...
        <svg role="img" aria-label={title} viewBox={`0 0 ${graphWidth} ${graphHeight}`} preserveAspectRatio="none" style={{ ...styles.graphCanvas, height: `${graphHeight}px` }}>
            ...
            {yMin < 0 && yMax > 0 ? <line x1={plot.left} y1={zeroY} x2={plot.right} y2={zeroY} stroke="rgba(238, 245, 255, 0.38)" strokeDasharray="4 4" strokeWidth="1" vectorEffect="non-scaling-stroke" /> : null}

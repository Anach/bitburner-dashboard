...
function formatWork(task) {
    if (!task || typeof task !== "object") return { label: "Idle", detail: "No focused work", affiliationType: "", affiliationName: "" };
    const durationSeconds = Math.max(0, Math.floor((Number(task.cyclesWorked) || 0) / 5));
    const duration = durationSeconds < 60
        ? `${durationSeconds}s`
        : `${Math.floor(durationSeconds / 60)}m ${durationSeconds % 60}s`;
    if (task.type === "COMPANY") return {
        label: `Company: ${task.companyName}`,
        detail: `Working ${duration}`,
        affiliationType: "company",
        affiliationName: String(task.companyName ?? ""),
    };
    if (task.type === "FACTION") return {
        label: `Faction: ${task.factionName}`,
        detail: `${task.factionWorkType} · ${duration}`,
        affiliationType: "faction",
        affiliationName: String(task.factionName ?? ""),
    };
...
export async function main(ns) {
    ...
    while (true) {
        ...
        try {
            ...
            work = { label: "Unavailable", detail: "Singularity req.", affiliationType: "", affiliationName: "" };
        }

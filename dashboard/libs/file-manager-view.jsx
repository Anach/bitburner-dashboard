...
function FilePane({ id, path, entries, selectedEntry, active, rowRefs, scrollRef, onScroll, onActivate, onSelect, onOpen }) {
    return (
        <section
...
function FileDialog({ dialog, onChangeTarget, onConfirm, onClose, onInputFocusChange }) {
    ...
    const requiresTarget = dialog.type === "copy" || dialog.type === "move";
    const danger = dialog.type === "delete";
            ...
            <section style={STYLES.modal} role="dialog" aria-modal="true" aria-label={title}>
                ...
                {!isPreview ? (
                    <div style={STYLES.modalActions}>
                        <button type="button" style={STYLES.modalButton} onClick={onClose}>Cancel</button>
                        <button
                            type="button"
                            style={{
                                ...STYLES.modalButton,
...
export function FileManagerView({
    ...
    const getSelectedEntry = (paneId) => {
        ...
        return entries.find((entry) => entry.id === panes[paneId]?.selectedId) ?? entries[0] ?? null;
    };
    ...
    }, [activePane, panes.left.path, panes.left.selectedId, panes.right.path, panes.right.selectedId, showIgnored, query, statusFilter, paneScrollTops.left, paneScrollTops.right]);
    ...
    const selectEntry = (paneId, selectedId) => {
        setPanes((current) => ({
            ...current,
            [paneId]: { ...current[paneId], selectedId },
        }));
    };
    ...
    const changeDirectory = (paneId, path) => {
        setPanes((current) => ({
            ...current,
            [paneId]: { path: normalizeFilePath(path), selectedId: "" },
        }));
    ...
    const moveSelection = (delta) => {
        ...
        selectEntry(activePane, entries[nextIndex].id);
    };
    ...
    const canCopy = Boolean(selectedEntry?.entryType === "file" && selectedEntry.exists && isEditableFile(selectedEntry.path));
    const canMove = Boolean(selectedEntry?.entryType === "file" && selectedEntry.exists && !selectedEntry.protected && isMovableFile(selectedEntry.path));
    const canArchive = canMove && !isPathInFolders(selectedEntry?.path, [archiveRoot]);
    const canDelete = Boolean(selectedEntry?.entryType === "file" && selectedEntry.exists && !selectedEntry.protected && isDeletableFile(selectedEntry.path));
    ...
    const keyActions = [
        ...
        { key: "F8", label: "Delete", enabled: canDelete, run: openDeleteDialog },
    ...
    const handleKeyDown = (event) => {
        ...
        if (event.key === "Home" || event.key === "End") {
            ...
            if (entry) selectEntry(activePane, entry.id);
            return;
        }
        if (event.key === "Enter") {
            event.preventDefault();
    ...
    const selectedMeta = selectedEntry?.entryType === "file"
        ? [
    ...
    return (
        <main data-dashboard-theme-role="app-frame" ref={shellRef} style={STYLES.shell} tabIndex={0} onKeyDown={handleKeyDown} onMouseDown={(event) => {
            ...
            <footer style={STYLES.footer}>
                <div style={{ minWidth: 0 }}>
                    <div style={STYLES.footerPath}>{selectedEntry?.path ? `home:/${selectedEntry.path}` : "home:/"}</div>

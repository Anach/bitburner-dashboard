...
function performDashboardKillAction(ns, command) {
    ...
    if (actionId === DASHBOARD_ACTION_IDS.KILL_PLUGIN_LIST_HOME_SCRIPTS
        || actionId === DASHBOARD_ACTION_IDS.KILL_PLUGIN_LIST_REMOTE_SCRIPTS) {
        const pluginFiles = new Set(command.pluginFiles);
        let supervisorStopped = false;
        if (actionId === DASHBOARD_ACTION_IDS.KILL_PLUGIN_LIST_HOME_SCRIPTS) {
            supervisorStopped = stopHomeScript(ns, SERVICE_SUPERVISOR_SCRIPT).status === "stopped";
        }
        const hosts = actionId === DASHBOARD_ACTION_IDS.KILL_PLUGIN_LIST_HOME_SCRIPTS ? ["home"] : remoteHosts;
        const result = killMatchingProcesses(ns, hosts, (filename) => pluginFiles.has(filename), excludeWorker);
        const location = hosts[0] === "home" ? "on home" : `across ${result.serverCount} remote server${result.serverCount === 1 ? "" : "s"}`;
        const supervisorSummary = supervisorStopped ? " Integration supervisor stopped." : "";
        return success(`Killed ${result.killedCount} plugin script${result.killedCount === 1 ? "" : "s"} ${location}.${supervisorSummary}`, "warning", result);
    }
